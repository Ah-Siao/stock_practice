...
yf.pdr_override()
start_year=st.selectbox('Please enter the starting year',[i for i in range(2013,2024)])
end_year=st.selectbox('Please enter the end year:',[i for i in range(2013,2024)])
view=st.multiselect('Please enter the properties you want to see individually:',['Open','Close','Volume','High','Low'])
...
try:
    ...
    df=pdr.get_data_yahoo(input_name, start=f'{start_year}-01-01', end=end_time)
except:
    input_name=f'{mystock}.TWO'
    df=pdr.get_data_yahoo(input_name, start=f'{start_year}-01-01', end=end_time)
